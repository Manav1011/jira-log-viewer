        ...
        if method == "GET":
            ...
            if rest_of_path.endswith("/worklog") or "/worklog?" in rest_of_path:
                ...
                while True:
                    ...
                    async with semaphore:
                        async with httpx.AsyncClient() as client:
                            response = await client.get(target_url, params=page_params, headers=headers)
            ...
            async with semaphore:
                async with httpx.AsyncClient() as client:
                    response = await client.post(target_url, params=params, headers=headers, json=body)
        elif method == "PUT":
            body = await request.json()
            async with semaphore:
                async with httpx.AsyncClient() as client:
                    response = await client.put(target_url, params=params, headers=headers, json=body)
        elif method == "DELETE":
            async with semaphore:
                async with httpx.AsyncClient() as client:
